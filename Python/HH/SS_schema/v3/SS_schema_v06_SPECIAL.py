...
INSERT_company_department = """INSERT INTO salarystat.company_department VALUES ({0}, {1}, 1, 'Магазин', NULL, NULL, NULL, 519, NULL, NULL);
INSERT INTO salarystat.company_department VALUES ({0}, {1}, 1, NULL, NULL, NULL, NULL, 519, NULL, NULL);
INSERT INTO salarystat.company_department VALUES ({0}, {1}, 1, NULL, NULL, NULL, NULL, 519, NULL, NULL);
INSERT INTO salarystat.company_department VALUES ({0}, {1}, 1, NULL, NULL, NULL, NULL, 519, NULL, NULL);
INSERT INTO salarystat.company_department VALUES ({0}, {1}, 1, NULL, NULL, NULL, NULL, 519, NULL, NULL);
INSERT INTO salarystat.company_department VALUES ({0}, {1}, 1, NULL, NULL, NULL, NULL, 519, NULL, NULL);
...
INSERT_benefit_policy = """INSERT INTO salarystat.benefit_policy VALUES ({0}, {1}, {2}, NULL, {3}, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);"""
...
TABLES = {'company_info' : INSERT_company_info,
          ...
          'benefit_policy' : INSERT_benefit_policy}

ORDER_OF_INSERTION = ['company_info', 'company_department', 'company_info_industry', 'salstat_lic_agr', 'department_emp_group', 'employee', 'benefit_policy']
...
def table_current_id(cursor, table):
    ...
    elif table == 'benefit_policy':
        cursor.execute("SELECT max(company_department_id) FROM salarystat.%s" % (table))
    else:
...
def run_script(cursor, table):
    ...
    for line in TABLES[table].split('\n'):
        if table == 'department_emp_group':
            ...
            #need to take max(company_department_id)
            query = line.format(table_id, company_department_id + 2)
            print query
            ...
            #need to take max(department_emp_group_id)
            query = line.format(table_id, table_id - 6)
            print query
            cursor.execute(query)
        elif table == 'benefit_policy':
            benefit_facility_code = 8 # Current count of the facilities groups
            company_department_code = table_id
            for facility_code in range(1, benefit_facility_code + 1):
                for department_code in range(company_department_code, company_department_code + 7):
                    if facility_code == 1:
                        query = line.format(facility_code, department_code, 'true', '30000')
                    else:
                        query = line.format(facility_code, department_code, 'false', 'NULL')
                    
                    print query
                    cursor.execute(query)
                    
        #need to take max(company-Info_id)
        elif table == 'company_department':
            query = line.format(table_id, table_id - 10)
            print query
            cursor.execute(query)
...
def update_sequence(cursor):
    for seq in SEQUENCE_LIST:
        current_id = table_current_id(cursor, seq)
